...
class Events:
    ...
    def one_moon(self):
        ...
        # 1 = reg patrol 2 = lifegen patrol 3 = ar patrol 4 = date
        ...
        with open(f"{resource_dir}ar.json",
                  encoding="ascii") as read_file:
            self.ar_txt = ujson.loads(read_file.read())
    ...
    def generate_events(self):
        ...
        if game.clan.your_cat.dead:
            if not game.clan.your_cat.ar and not game.clan.your_cat.outside:
                resource_dir = "resources/dicts/events/lifegen_events/events_dead_sc/"
            elif game.clan.your_cat.ar and not game.clan.your_cat.outside:
                resource_dir = "resources/dicts/events/lifegen_events/events_dead_ar/"
            elif not game.clan.your_cat.ar and game.clan.your_cat.ID in game.clan.unknown_cats:
    ...
    def generate_ar_events(self):
        if random.randint(1,3) == 1:
            evt = self.process_text(random.choice(self.ar_txt["general"]))
            if evt:
                involved_cats = []
                involved_cats.append(game.clan.your_cat.ID)
                for i in self.cat_dict.items():
        ...
        if random.randint(1,30) == 1:
            ...
            counter = 0
            while r_clanmate.dead or r_clanmate.outside or r_clanmate.status in ['kitten', 'newborn', 'deputy', 'leader'] or r_clanmate.joined_ar or r_clanmate.ID == game.clan.your_cat.ID:
                counter+=1
                if counter > 15:
                    return
                r_clanmate = Cat.all_cats.get(random.choice(game.clan.clan_cats))
            
            r_clanmate.joined_ar = True
            r_clanmate.faith -= 1
            r_clanmate.update_ar_mentor()
            self.cat_dict["c_m"] = r_clanmate
            evt_txt = random.choice(self.ar_txt["clanmate"])
    ...
    def perform_ceremonies(self, cat):
        ...
        if game.clan.deputy:
            if game.clan.deputy is not None and \
                ...
                if cat.outside and not (cat.exiled or cat.ar) and cat.status not in ['kittypet', 'loner', 'rogue','former Clancat']:
                    shuntext = ""
