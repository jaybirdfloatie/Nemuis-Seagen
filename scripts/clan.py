...
class Clan:
    ...
    def create_clan(self):
        ...
        if self.clan_age == "new":
            self.demon.backstory = choice(BACKSTORIES["backstory_categories"]["new_ar_guide_backstories"])
        else:
            self.demon.backstory = choice(BACKSTORIES["backstory_categories"]["ar_backstories"])
        self.add_cat(self.demon)
        self.add_to_abyssalreef(self.demon)
        self.all_clans = []
    ...
    def populate_ar(self):
        for i in range(randint(0,5)):
            ...
            ar_cats[0].history.beginning = None
            ar_cats[0].dead_for = randint(20, 200)
    ...
    def add_to_starclan(self, cat):  # Same as add_cat
        ...
        if (
            ...
            if cat.ID in self.abyssalreef_cats:
                self.abyssalreef_cats.remove(cat.ID)
    ...
    def add_to_abyssalreef(self, cat):  # Same as add_cat
        """
        Places the dead cat into the Abyssal Reef.
        It should not be removed from the list of cats in the clan
        """
        if cat.ID in Cat.all_cats and cat.dead and cat.ar:
            self.abyssalreef_cats.append(cat.ID)
    ...
    def remove_cat(self, ID):  # ID is cat.ID
        ...
        if ID in self.abyssalreef_cats:
            self.abyssalreef_cats.remove(ID)
    ...
    def load_clan_json(self):
        ...
        for cat in clan_data["clan_cats"].split(","):
            if cat in Cat.all_cats:
                game.clan.add_cat(Cat.all_cats[cat])
                game.clan.add_to_starclan(Cat.all_cats[cat])
                game.clan.add_to_abyssalreef(Cat.all_cats[cat])
