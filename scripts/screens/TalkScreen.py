...
class TalkScreen(Screens):
    ...
    def update_camp_bg(self):
        ...
        for leaf in leaves:
            ...
            elif self.the_cat.dead and not self.the_cat.outside and not self.the_cat.ar:
            ...
            elif self.the_cat.dead and not self.the_cat.outside and self.the_cat.ar:
                platform_dir = "resources/images/dead_camps/arbackground_eclipse.png"
            else:
    ...
    def filter_texts(self, cat, possible_texts):
        ...
        for talk_key, talk in possible_texts.items():
            ...
            if "sc_faith" in tags and cat.faith < 0:
                continue
            if "ar_faith" in tags and cat.faith > 0:
                continue
            ...
            if not any(t in tags for t in ["they_dead", "they_sc", "they_ar", "they_ur"]) and cat.dead:
                continue
            if not any(t in tags for t in ["you_dead", "you_sc", "you_ar", "you_ur"]) and you.dead:
                continue

            if any(t in tags for t in ["they_dead", "they_sc", "they_ar", "they_ur"]) and not cat.dead:
                continue
            if any(t in tags for t in ["you_dead", "you_sc", "you_ar", "you_ur"]) and not you.dead:
                continue
            ...
            if "you_artrainee" in tags and not you.joined_ar:
                continue

            if "they_artrainee" in tags and not cat.joined_ar:
                continue

            if "you_not_artrainee" in tags and cat.joined_ar:
                continue
                
            if "they_not_artrainee" in tags and cat.joined_ar:
                continue

            if "they_ar" in tags and (not cat.ar or cat.outside):
                continue
            if "you_ar" in tags and (not you.ar or you.outside):
                continue
            if "they_sc" in tags and (cat.ar or cat.outside):
                continue
            if "you_sc" in tags and (you.ar or you.outside):
                continue
            ...
            if any(i in [
                ...
                "from_mentor", "from_ar_mentor", "from_your_kit", "from_your_apprentice",
                "from_ar_apprentice", "from_mate", "from_parent", "adopted_parent", "from_kit",
                ...
                if "from_mentor" in tags:
                    if you.mentor == cat.ID:
                        fam = True
                if "from_ar_mentor" in tags:
                    if you.ar_mentor == cat.ID:
                        fam = True
                if "from_your_apprentice" in tags:
                    if cat.mentor == you.ID:
                        fam = True
                if "from_ar_apprentice" in tags:
                    if cat.ar_mentor == you.ID:
                        fam = True
            ...
            if you.dead or cat.dead:
                ...
                if you.dead:
                    for tag in tags:
                        if tag.startswith("min_you_deaaror_"):
                            min_value = int(tag.split("_")[-1])
                            if you.dead_for < min_value:
                                skip_processing = True
                                break
                        elif tag.startswith("max_you_deaaror_"):
                            max_value = int(tag.split("_")[-1])
                ...
                if cat.dead and not skip_processing:
                    for tag in tags:
                        if tag.startswith("min_they_deaaror_"):
                            min_value = int(tag.split("_")[-1])
                            if cat.dead_for < min_value:
                                skip_processing = True
                                break
                        elif tag.startswith("max_they_deaaror_"):
                            max_value = int(tag.split("_")[-1])
    ...
    def load_and_replace_placeholders(self, file_path, cat, you):
        with open(file_path, 'r') as read_file:
            ...
            add_on = add_on_map.get((you.dead, you.ar, you.outside), "")
            if "grief stricken" in you.illnesses:
                add_on += " g"
            if you.shunned > 0:
                add_on += " sh"
            ...
            if "deaf" in you.permanent_condition:
                add_on += " d"
            y_c_text += add_on
            add_on2 = add_on_map.get((cat.dead, cat.ar, cat.outside), "")
            if "grief stricken" in cat.illnesses:
                add_on2 += " g"
            if cat.shunned > 0:
                add_on2 += " sh"
    ...
    def choose_text(self, cat, texts_list):
        ...
        weighted_tags = [
            ...
            "from_ar_mentor", "from_your_kit", "from_your_apprentice",
            "from_ar_apprentice", "from_mate", "from_parent", "adopted_parent",
