...
class PatrolScreen(Screens):
    ...
    cat_icon = pygame.transform.scale(pygame.image.load("resources/images/buttons/cat_icon.png").convert_alpha(),
                                    (100, 100))
    ar_icon = pygame.transform.scale(pygame.image.load("resources/images/buttons/ar_toggle2.png").convert_alpha(),
                                    (100, 100))
    ...
    def handle_choose_cats_events(self, event):
        ...
        if 'cat_icon' in self.elements and event.ui_element == self.elements['cat_icon']:
            ...
            if not game.clan.your_cat.dead and not game.clan.your_cat.outside and game.clan.your_cat.joined_ar:
                self.elements['ar_icon'].enable()
            else:
                self.elements['ar_icon'].disable()
        ...
        elif 'ar_icon' in self.elements and event.ui_element == self.elements['ar_icon']:
            game.switches["patrol_category"] = "ar"
            self.selected_cat = None
            self.current_patrol.clear()
    ...
    def update_button(self):
        ...
        # default is six
        if game.switches['patrol_category'] == "ar":
            self.max_cats = 2
        ...
        if self.patrol_stage == 'choose_cats':
            ...
            if game.switches["patrol_category"] == "lifegen":
                text = "lifegen"
            elif game.switches["patrol_category"] == "ar":
                text = "abyssal reef"
            elif game.switches["patrol_category"] == "date":
                text = "date"
            else:
    ...
    def open_choose_cats_screen(self):
        ...
        self.elements['ar_icon'] = UIImageButton(scale(pygame.Rect((726, 1120), (68, 68))), "",
                                                object_id="#ar_toggle", manager=MANAGER)
        if not game.clan.your_cat.dead and not game.clan.your_cat.outside and game.clan.your_cat.joined_ar and game.switches['patrol_category'] != "ar":
            self.elements['ar_icon'].enable()
        else:
            self.elements['ar_icon'].disable()
