...
class StarClanScreen(Screens):
    ...
    def handle_event(self, event):
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.abyssal_reef_button:
                self.change_screen('abyssal reef screen')
    ...
    def get_dead_cats(self):
        ...
        for the_cat in Cat.all_cats_list:
            if the_cat.dead and the_cat.ID != game.clan.instructor.ID and the_cat.ID != game.clan.demon.ID and not the_cat.outside and not the_cat.ar and not the_cat.faded:
