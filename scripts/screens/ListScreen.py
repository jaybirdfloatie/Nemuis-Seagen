...
class ListScreen(Screens):
        ...
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            ...
            # CHOOSE GROUP
                ...
                if element == self.choose_group_buttons["view_your_clan_button"]:
                    ...
                    self.get_ur_cats()
                elif element == self.choose_group_buttons["view_abyssal_reef_button"]:
                    self.get_ar_cats()
                self.update_cat_list(
                    self.cat_list_bar_elements["search_bar_entry"].get_text()
                )
    ...
    def screen_switches(self):
        ...
        self.ar_bg = pygame_gui.elements.UIImage(
            scale(pygame.Rect((0, 0), (1600, 1400))),
            pygame.transform.scale(
                pygame.image.load("resources/images/abyssalreefbg.png").convert_alpha(),
                (1600, 1400),
            ),
            container=self.list_screen_container,
            starting_height=1,
            object_id="#abyssal_reef_bg",
            visible=False,
            manager=MANAGER,
        )
    ...
    def get_ar_cats(self):
        self.current_group = 'ar'
        self.death_status = 'dead'
        self.full_cat_list = []

        for the_cat in Cat.all_cats_list:
            if the_cat.dead and the_cat.ID != game.clan.demon.ID and the_cat.ID != game.clan.instructor.ID and the_cat.ar and \
                    not the_cat.faded:
                self.full_cat_list.append(the_cat)
    ...
    def get_cat_list(self):
        ...
        if game.last_list_forProfile:
            if game.last_list_forProfile == "sc":
                self.get_sc_cats()
            elif game.last_list_forProfile == "ar":
                self.get_ar_cats()
