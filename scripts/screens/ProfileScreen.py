...
class ProfileScreen(Screens):
    ...
    def __init__(self, name=None):
        ...
        self.join_ar_button = None
        self.exit_ar_button = None
    ...
    def handle_tab_events(self, event):
        ...
        # Dangerous Tab
            ...
            if event.ui_element == self.kill_cat_button:
            ...
            elif event.ui_element == self.join_ar_button:
                game.clan.your_cat.joined_ar = True
                game.clan.your_cat.faith -=1
                game.clan.your_cat.update_ar_mentor()
                self.join_ar_button.disable()
                self.clear_profile()
                self.build_profile()
            elif event.ui_element == self.exit_ar_button:
                game.clan.your_cat.joined_ar = False
                game.clan.your_cat.faith += 1
                try:
                    Cat.all_cats[game.clan.your_cat.ar_mentor].ar_apprentices.remove(game.clan.your_cat.ID)
                except:
                    print("ERROR: removing ar apprentice")
                game.clan.your_cat.ar_mentor = None
                self.exit_ar_button.disable()
                self.clear_profile()
                self.build_profile()
                ...
                # if not self.the_cat.outside and not self.the_cat.ar:
                #     object_id = "#exile_ar_button"
                # elif self.the_cat.ar and not self.the_cat.outside:
                ...
                #     object_id = "#guide_sc_button"
                    if self.the_cat.ID != game.clan.instructor.ID and self.the_cat.ID != game.clan.demon.ID:
                        if event.ui_object_id == "#guide_sc_button":
                            ...
                            self.the_cat.ar = False
                            game.clan.add_to_starclan(self.the_cat)
                            self.the_cat.thought = "Is relieved to once again hunt in StarClan"
                        elif event.ui_object_id == "#exile_ar_button":
                            self.the_cat.outside, self.the_cat.exiled = False, False
                            self.the_cat.ar = True
                            game.clan.add_to_abyssalreef (self.the_cat)
    ...
    def build_profile(self):
        ...
        is_ar_instructor = False
        if self.the_cat is None:
            return
        if (
            self.the_cat.dead
            and game.clan.instructor.ID == self.the_cat.ID
            and self.the_cat.ar is False
        ):
            is_sc_instructor = True
        elif self.the_cat.dead and game.clan.demon.ID == self.the_cat.ID and self.the_cat.ar is True:
            is_ar_instructor = True
        ...
        if is_ar_instructor:
            if game.clan.followingsc == True:
                self.the_cat.thought = "Wants to drag the cats of " + game.clan.name + "Clan into the Abyssal Reef"
                self.the_cat.ar
            else:
    ...
    def determine_previous_and_next_cat(self):
        ...
        if self.the_cat.dead and not is_instructor and not self.the_cat.ar and \
        ...
        if self.the_cat.dead and not is_demon and self.the_cat.ar and \
                not (self.the_cat.outside or self.the_cat.exiled):
        ...
        for check_cat in Cat.ordered_cat_list:
            if check_cat.ID == self.the_cat.ID:
                ...
                if current_cat_found == 0 and check_cat.ID != self.the_cat.ID and check_cat.dead == self.the_cat.dead \
                        ...
                        check_cat.ar == self.the_cat.ar and not check_cat.faded and check_cat.moons > -1:
    ...
    def generate_column2(self, the_cat):
        ...
        # STATUS
        if the_cat.outside and not (the_cat.exiled or the_cat.ar) and the_cat.status not in ['kittypet', 'loner', 'rogue',
            ...
            if game.settings['dark mode']:
                output += "<font color='#FF9999' >" + "Abyssal Reef "+ the_cat.status + "</font>"
            else:
                output += "<font color='#950000' >" + "Abyssal Reef "+ the_cat.status + "</font>"
        elif the_cat.dead and not the_cat.ar and not the_cat.outside:
            if game.settings['dark mode']:
                output += "<font color ='#A8BBFF'>" "StarClan " + the_cat.status + "</font>"
            else:
                output += "<font color ='#2B3DC3'>" "StarClan " + the_cat.status + "</font>"
        elif the_cat.dead and not the_cat.ar and the_cat.outside:
            if game.settings['dark mode']:
                output += "<font color ='#CE9arF'>" "ghost " + the_cat.status + "</font>"
            else:
        ...
        if the_cat.ar_mentor and not the_cat.dead:
            mentor_ob = Cat.fetch_cat(the_cat.ar_mentor)
            if mentor_ob:
                output += "abyssal reef mentor: " + str(mentor_ob.name) + "\n"
        ...
        if the_cat.ar_apprentices and the_cat.dead:
            app_count = len(the_cat.ar_apprentices)
            if app_count == 1 and Cat.fetch_cat(the_cat.ar_apprentices[0]) and not Cat.fetch_cat(the_cat.ar_apprentices[0]).dead:
                output += 'abyssal reef apprentice: ' + str(Cat.fetch_cat(the_cat.ar_apprentices[0]).name)

                # NEWLINE ----------
                output += "\n"
            elif app_count > 1:
                output += 'abyssal reef apprentices: ' + ", ".join([str(Cat.fetch_cat(i).name) for i in the_cat.ar_apprentices if Cat.fetch_cat(i) and not Cat.fetch_cat(i).dead])
    ...
    def get_backstory_text(self):
        ...
        if self.the_cat.history and self.the_cat.history.wrong_placement and self.the_cat.dead and not self.the_cat.outside:
            if self.the_cat.ar:
                text += f"<br>{self.the_cat.name} was wrongly placed in the abyssal reef."
            else:
    ...
    def open_faith_tab(self):
        ...
        if self.the_cat.lock_faith == "flexible":
            ...
            if cat_faith > 9:
                ...
                cat_faith = 1
        elif self.the_cat.lock_faith == "abyssal reef":
            if cat_faith > -1:
                cat_faith = -1
            elif cat_faith < -9:
                cat_faith = 9
    ...
    def toggle_dangerous_tab(self):
        ...
        if previous_open_tab == "dangerous":
            ...
            if game.clan.your_cat.joined_ar:
                self.exit_ar_button = UIImageButton(
                scale(pygame.Rect((1156, 1115), (344, 72))),
                "",
                object_id="#exit_ar_button",
                tool_tip_text='Leave the abyssal reef',
                starting_height=2, manager=MANAGER
                )
            else:
                self.join_ar_button = UIImageButton(
                scale(pygame.Rect((1156, 1115), (344, 72))),
                "",
                object_id="#join_ar_button",
                tool_tip_text='Join the abyssal reef',
                starting_height=2, manager=MANAGER
            )
    ...
    def update_disabled_buttons_and_text(self):
        ...
        # Dangerous Tab
            ...
            if self.the_cat.ID != game.clan.your_cat.ID:
                ...
                if self.join_ar_button:
                    self.join_ar_button.hide()
                if self.exit_ar_button:
                    self.exit_ar_button.hide()
                self.affair_button.hide()
            else:
                self.murder_cat_button.show()
                if self.join_ar_button:
                    self.join_ar_button.show()
                if self.exit_ar_button:
                    self.exit_ar_button.show()
    ...
    def close_current_tab(self):
        ...
        if self.open_tab is None:
            ...
            if self.join_ar_button:
                self.join_ar_button.kill()
            if self.exit_ar_button:
                self.exit_ar_button.kill()
    ...
    def get_platform(self):
        ...
        order = ["beach", "forest", "mountainous", "nest", "plains", "SC/ar"]
        ...
        if the_cat.ar:
            biome_platforms = platformsheet.subsurface(
                pygame.Rect(0, order.index("SC/ar") * 70, 640, 70)
            )
    ...
    def get_dead_cat_talk(self):
        ...
        # They ar
        cat_dead_condition_ar = (
            self.the_cat.dead and
            self.the_cat.ar and
            (game.clan.your_cat.dead or
            (game.clan.your_cat.skills.meets_skill_requirement(SkillPath.ABYSS)
            and game.clan.your_cat.moons >=1) or
            game.clan.your_cat.joined_ar)
            )
        ...
        # you ar
        cat_alive_condition_ar = (
            game.clan.your_cat.dead and
            game.clan.your_cat.ar and
            (self.the_cat.dead or
            (self.the_cat.skills.meets_skill_requirement(SkillPath.ABYSS) and
            self.the_cat.moons >= 1) or
            self.the_cat.joined_ar)
            )
        ...
        if self.the_cat.dead and not self.the_cat.outside and not self.the_cat.ar:
            if not cat_dead_condition_sc:
                return False
            else:
                return True
            
        if self.the_cat.dead and self.the_cat.outside and not self.the_cat.ar:
            if not cat_dead_condition_ur:
                return False
            else:
                return True
            
        if self.the_cat.dead and not self.the_cat.outside and self.the_cat.ar:
            if not cat_dead_condition_ar:
                return False
            else:
                return True
            
        if game.clan.your_cat.dead:
            if game.clan.your_cat.ar:
                if not cat_alive_condition_ar:
                    return False
                else:
                    return True
