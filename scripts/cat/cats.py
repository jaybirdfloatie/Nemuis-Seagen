    ...
    def die(self, body: bool = True):
        ...
        if self.status == "leader":
            if game.clan.leader_lives > 0:
                ...
                death_thought = Thoughts.leader_death_thought(self, 0, abyssalreef)
                final_thought = event_text_adjust(self, death_thought, main_cat=self)
                self.thought = final_thought
                if game.clan.followingsc is True:
                    ...
                    text = "They've lost their last life and have travelled to the Abyssal Reef."
        else:
            self.dead = True
            game.just_died.append(self.ID)
            death_thought = Thoughts.new_death_thought(self, abyssalreef, isoutside)
            final_thought = event_text_adjust(self, death_thought, main_cat=self)
            self.thought = final_thought
        ...
        if not self.outside and self.dead_for < 2:
            ...
            if self.history and self.history.murder and "is_murderer" in self.history.murder and len(self.history.murder["is_murderer"]) > 2:
                self.ar = True
                game.clan.add_to_abyssalreef(self)
            elif self.faith <= -9:
                self.ar = True
                game.clan.add_to_abyssalreef(self)
            elif self.faith >= 9:
...
# pylint: disable=f-string-without-interpolation
    ...
    def generate_lead_ceremony(self):
        ...
        if game.clan.followingsc == False:
            starclan = False
            ceremony_dict = LEAD_CEREMONY_ar
        else:
            starclan = True
        ...
        for rel in relationships:
            ...
            if kitty and kitty.dead and kitty.status != "newborn":
                ...
                if starclan:
                    ...
                    if kitty.ID not in game.clan.abyssalreef_cats:
                        continue
        ...
        if len(life_givers) < 8:
            ...
            if starclan:
                ...
                possible_ar_cats = [
                    i
                    for i in game.clan.abyssalreef_cats
                    if self.fetch_cat(i)
                    and i not in life_givers
                    and self.fetch_cat(i).status not in ["leader", "newborn"]
                ]
                if len(possible_ar_cats) - 1 < amount:
                    extra_givers = possible_ar_cats
                else:
                    extra_givers = sample(possible_ar_cats, k=amount)
        ...
        if not life_giving_leader:
            ...
            if coin_flip == 1:
                ...
                if starclan:
                    ...
                    for kitty in reversed(game.clan.abyssalreef_cats):
                        if (
                            self.fetch_cat(kitty)
                            and self.fetch_cat(kitty).status == "leader"
                        ):
                ...
                if starclan:
                    ...
                    for kitty in game.clan.abyssalreef_cats:
                        if (
                            self.fetch_cat(kitty)
                            and self.fetch_cat(kitty).status == "leader"
                        ):
    ...
    def thoughts(self):
        ...
        if not self.dead and not self.outside:
            where_kitty = "inside"
        elif self.dead and not self.ar and not self.outside:
            where_kitty = "starclan"
        elif self.dead and self.ar:
            where_kitty = "hell"
    ...
    def update_ar_mentor(self):
        """Handles giving clan members ar mentors"""
        if not self.joined_ar or self.dead or self.ar_mentor:
            return
        
        potential_mentors = []
        for c in Cat.all_cats_list:
            if c.dead and c.ar and c.moons >= 6 and self.ID != c.ID:
                potential_mentors.append(c)

        priority_mentors = []
        for c in potential_mentors: 
            if len(self.ar_apprentices) == 0:
                priority_mentors.append(c)

        if priority_mentors:
            ar_mentor = choice(priority_mentors)
            self.ar_mentor = ar_mentor.ID
            ar_mentor.ar_apprentices.append(self.ID)
        elif potential_mentors:
            ar_mentor = choice(potential_mentors)
            self.ar_mentor = ar_mentor.ID
            ar_mentor.ar_apprentices.append(self.ID)
    ...
    def load_faded_cat(cat: str):
        ...
        cat_ob = Cat(
            ...
            faded=True,
            ar=cat_info["ar"] if "ar" in cat_info else False,
        )
        if cat_info["parent1"]:
    ...
    def get_save_dict(self, faded=False):
        if faded:
            ...
            return {
                ...
                "joined_ar": self.joined_ar if self.joined_ar else False,
                ...
                "ar_mentor": self.ar_mentor if self.ar_mentor else None,
                "ar_apprentices": self.ar_apprentices if self.ar_apprentices else [],
...
with open("resources/dicts/lead_ceremony_ar.json", "r", encoding="utf-8") as read_file:
    LEAD_CEREMONY_ar = ujson.loads(read_file.read())
