...
class Thoughts:
    ...
    def cats_fulfill_thought_constraints(main_cat, random_cat, thought, game_mode, biome, season, camp) -> bool:
        ...
        if 'main_faith_constraint' in thought:
            ...
            if "low_ar" in thought['main_faith_constraint']:
                if (not main_cat.faith < 0 and main_cat.faith > -3):
                    return False
            elif "mid_ar" in thought['main_faith_constraint']:
                if (not main_cat.faith < -3 and main_cat.faith > -6):
                    return False
            elif "high_ar" in thought['main_faith_constraint']:
        ...
        if 'random_faith_constraint' in thought:
            ...
            if "low_ar" in thought['random_faith_constraint']:
                if (not random_cat.faith < 0 and random_cat.faith > -3):
                    return False
            elif "mid_ar" in thought['random_faith_constraint']:
                if (not random_cat.faith < -3 and random_cat.faith > -6):
                    return False
            elif "high_ar" in thought['random_faith_constraint']:
        ...
        if random_cat and 'random_living_status' in thought:
            ...
            elif random_cat and random_cat.dead and random_cat.ar:
                living_status = "abyssalreef"
            elif random_cat and random_cat.dead and not random_cat.ar:
    ...
    def load_thoughts(main_cat, other_cat, game_mode, biome, season, camp):
        ...
        elif main_cat.dead and not main_cat.outside and not main_cat.ar:
            spec_dir = "/starclan"
        elif main_cat.dead and main_cat.ar:
            spec_dir = "/abyssalreef"
        elif main_cat.dead and main_cat.outside and not main_cat.ar:
    ...
    def leader_death_thought(self, lives_left, abyssalreef):
        """
        ...
        :param abyssalreef: Whether or not dead cats go to StarClan (false) or the ar (true)
        """
        base_path = f"resources/dicts/thoughts/ondeath"
        if abyssalreef is False:
            spec_dir = "/starclan"
        elif abyssalreef:
            spec_dir = "/abyssalreef"
        THOUGHTS: []
    ...
    def new_death_thought(self, abyssalreef, isoutside):
        base_path = f"resources/dicts/thoughts/ondeath"
        if isoutside:
            spec_dir = "/unknownresidence"
        elif abyssalreef is False:
            spec_dir = "/starclan"
        elif abyssalreef:
            spec_dir = "/abyssalreef"
        THOUGHTS: []
