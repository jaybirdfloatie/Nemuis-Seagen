...
def create_new_cat_block(
    ...
    for _tag in attribute_list:
        ...
        if match.group(1) == "ur":
            ...
            break
        elif match.group(1) == "ar":
            ar = True
            outside = False
            alive = False
            thought = "Is annoyed with the living cat they just met"
            chosen_backstory = choice(BACKSTORIES["backstory_categories"]["ar_backstories"])
            encountered_dead_ar = True
    ...
    if not chosen_cat:
        ...
        for n_c in new_cats:
            ...
            if encountered_dead_ar or encountered_dead_sc or encountered_dead_ur:
...
def create_new_cat(
    ...
    for index in range(number_of_cats):
        ...
        if ar:
            new_cat.ar = True
        # give apprentice aged cat a mentor
        # this is in a weird spot but ar cats were getting clancat mentors otherwise
...
def get_leader_life_notice() -> str:
    ...
    if lives > 0:
        ...
        if game.clan.followingsc:
            ...
            text = 'The leader has no lives left and has travelled to the Abyssal Reef.'
...
def generate_sprite(
    ...
    try:
        ...
        if not dead:
            ...
            new_sprite.blit(sprites.sprites["lineartar" + cat_sprite], (0, 0))
        ...
        if (
            ...
            if cat.ar:
                temp = sprites.sprites["fadear" + stage + cat_sprite].copy()
...
def adjust_txt(Cat, text, cat, cat_dict, r_c_allowed, o_c_allowed):
    ...
    if "ar_m_n" in text:
        cluster = False
        rel = False
        match = re.search(r'ar_m_n(\w+)', text)
        if match:
            x = match.group(1).strip("_")
            cluster = True
        else:
            x = ""
        match2 = re.search(r'(\w+)ar_m_n', text)
        if match2:
            r = match2.group(1).strip("_")
            rel = True
        else:
            r = ""
        if you.joined_ar and not you.dead and you.ar_mentor and cat.ID != you.ar_mentor and not Cat.all_cats.get(you.ar_mentor) is None:
            text, in_dict = cat_dict_check("ar_m_n", cluster, x, rel, r, text, cat_dict)
            cat_dict["ar_m_n"] = Cat.all_cats.get(you.ar_mentor)
            text = add_to_cat_dict("ar_m_n", cluster, x, rel, r, Cat.fetch_cat(you.ar_mentor), text, cat_dict)
        else:
            return ""
    ...
    if "t_ar_mn" in text:
        cluster = False
        rel = False
        match = re.search(r't_ar_mn(\w+)', text)
        if match:
            x = match.group(1).strip("_")
            cluster = True
        else:
            x = ""
        match2 = re.search(r'(\w+)t_ar_mn', text)
        if match2:
            r = match2.group(1).strip("_")
            rel = True
        else:
            r = ""
        if cat.joined_ar and not cat.dead and cat.ar_mentor:
            text, in_dict = cat_dict_check("t_ar_mn", cluster, x, rel, r, text, cat_dict)
            cat_dict["t_ar_mn"] = Cat.all_cats.get(cat.ar_mentor)
            addon_check = abbrev_addons(cat, Cat.fetch_cat(cat.ar_mentor), cluster, x, rel, r)
        if addon_check is False:
            return ""
        text = add_to_cat_dict("t_ar_mn", cluster, x, rel, r, Cat.fetch_cat(cat.ar_mentor), text, cat_dict)
    ...
    # Random abyssal reef cat
    if "rar_c" in text:
        cluster = False
        rel = False
        match = re.search(r'rar_c(\w+)', text)
        if match:
            x = match.group(1).strip("_")
            cluster = True
        else:
            x = ""
        match2 = re.search(r'(\w+)rar_c', text)
        if match2:
            r = match2.group(1).strip("_")
            rel = True
        else:
        ...
        text, in_dict = cat_dict_check("rar_c", cluster, x, rel, r, text, cat_dict)
        

        if in_dict is False:
            random_cat = Cat.all_cats.get(choice(game.clan.abyssalreef_cats))
            addon_check = abbrev_addons(cat, random_cat, cluster, x, rel, r)
            ...
            text = add_to_cat_dict("rar_c", cluster, x, rel, r, random_cat, text, cat_dict)
