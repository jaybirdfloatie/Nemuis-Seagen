...
class Patrol:
    ...
    def add_patrol_cats(self, patrol_cats: List[Cat], clan: Clan) -> None:
        ...
        if "patrol_category" in game.switches and game.switches["patrol_category"] in ["ar", "date", "lifegen"]:
        ...
        if "patrol_category" in game.switches and game.switches["patrol_category"] == 'date':
            for date_cat in patrol_cats:
                if date_cat.ID != game.clan.your_cat.ID:
                    ...
                    break
        elif "patrol_category" in game.switches and len(patrol_cats) > 1 and game.switches["patrol_category"] == 'ar':
    ...
    def get_possible_patrols(
        ...
        if game.switches["patrol_category"] == 'clangen':
            ...
            possible_patrols.extend(self.generate_patrol_events(self.ar_lifegen))
    ...
    def _filter_patrols(
        ...
        for patrol in possible_patrols:
            ...
            if game.switches["patrol_category"] in ['lifegen', 'ar', 'date']:
                if game.switches["patrol_category"] == "ar":
                    if len(self.patrol_cats) > 1:
                        other_cat = self.patrol_cats[1]
                        
                        if not other_cat.joined_ar:
                    ...
                    if "shunned" not in patrol.tags and "ar" not in patrol.tags:
                        if game.clan.your_cat.shunned > 0:
                            continue
                if game.switches["patrol_category"] == "date":
                    if "ar" in patrol.tags:
                        if len(self.patrol_cats) > 1:
                            other_cat = self.patrol_cats[1]
                            if not game.clan.your_cat.joined_ar or not other_cat.joined_ar:
    ...
    def update_resources(self, biome_dir, leaf):
        ...
        if game.switches["patrol_category"] == 'clangen':
            ...
            self.ar_lifegen = None
            with open(f"{resource_dir}/lifegen/ar.json", 'r', encoding='ascii') as read_file:
                self.ar_lifegen = ujson.loads(read_file.read())
        elif game.switches["patrol_category"] == 'date':
